...
- [x] 5.1 创建 `lib/api/notifications.ts`：`fetchNotifications(page, size)`、`markAsRead(id)`、`markAllRead()`、`fetchUnreadCount()`
- [x] 5.2 定义 TypeScript 类型：`NotificationData`、`NotificationItem`、`UnreadCountData`、`MarkAllReadData`

## 6. 前端 — notifications Zustand store

- [x] 6.1 创建 `lib/stores/notifications.ts`：`unreadCount`、`setUnreadCount`、`decrementUnread`、`resetUnread`、`fetchUnreadCount`
- [x] 6.2 编写 notifications store 测试

## 7. 前端 — 通知列表页 /notifications

- [x] 7.1 创建 `app/notifications/page.tsx`（Client Component，含分页 + 状态管理）
- [x] 7.2 创建 `app/notifications/_components/NotificationItem.tsx`（单条通知，含 unread/read/hover/actor-deleted/target-deleted 5 种状态）
- [x] 7.3 创建 `app/notifications/_components/NotificationTypeIcon.tsx`（4 种类型图标 + 颜色映射）
- [x] 7.4 实现四态覆盖：Loading 骨架（6 条）/ Content / Empty（BellOff 引导）/ Error（AlertCircle + Retry）
- [x] 7.5 实现分页加载（Intersection Observer + loadMore 状态：loading/error/hasMore）
- [x] 7.6 实现 "Mark all as read" 按钮（enabled/disabled/submitting/success/error 5 种状态 + 乐观更新）
- [x] 7.7 实现点击通知条目行为（未读 → 标记已读乐观更新 + 跳转；已删除 target → toast 不跳转）
- [x] 7.8 实现相对时间显示（just now / Xm / Xh / Xd / Xw / 日期）
- [x] 7.9 实现响应式布局（Mobile 紧凑头部 + icon-only 按钮 / Tablet+ 完整头部）
- [x] 7.10 编写 `/notifications` 页面测试

## 8. 前端 — NotificationBell 导航栏组件

- [x] 8.1 创建 `app/_components/NotificationBell.tsx`（Client Component，Bell icon + 未读 badge）
- [x] 8.2 实现 8 种状态：未登录不渲染 / 0 未读无 badge / 1-9 / 10-99 / 100+（99+）/ hover / loading / fetch 失败静默
- [x] 8.3 实现未读数刷新策略（挂载 + visibilitychange + 乐观更新 + 60s 轮询）
- [x] 8.4 导航栏集成 NotificationBell（位于 UserDropdown 左侧）
- [x] 8.5 编写 NotificationBell 组件测试

## 9. 前端 — 路由守卫

- [x] 9.1 `middleware.ts` 的 `PROTECTED_PAGES` 新增 `/notifications`
- [x] 9.2 编写 middleware 测试更新
